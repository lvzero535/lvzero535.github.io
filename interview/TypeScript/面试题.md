...
## 3. TypeScript 中interface和type的区别是什么？

1. interface 可以被合并（declaration merging）， 多次声明会合并到一起，而 type 不能。
2. type 可以表示原始类型（如：string、number、boolean），联合类型，元组，而 interface 不能。
3. 都可以定义对象和函数类型，但type可以为任何类型创建别名。
