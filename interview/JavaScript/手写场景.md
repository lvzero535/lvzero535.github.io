...
console.log(isCircleDeps(data1)) // true
...
console.log(isCircleDeps(data2)) // false

```

```js
function isCircleDeps(data) {
    const graph = new Map();
    data.forEach(pkg => {
        graph.set(pkg.name, pkg.node_modules.map(dep => dep.name));
    });
